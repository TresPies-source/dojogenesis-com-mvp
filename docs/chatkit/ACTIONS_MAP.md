...
| `dojo.copy.last_prompt` | "Copy last prompt" | Copies the user's most recent message to the clipboard | Extract last user message from DOM, copy to clipboard via `navigator.clipboard.writeText()`, show "Copied ✓" feedback |
| `dojo.copy.last_prompt` (with context) | "Copy last prompt (with context)" | Copies the last user message with timestamp and assistant summary | Extract last user message and assistant response, format with metadata in markdown, copy to clipboard |
| `dojo.copy.last_exchange` | "Copy last exchange (prompt + reply)" | Copies both the last user prompt and assistant response | Extract last user and assistant messages, format in markdown with headers, copy to clipboard |
